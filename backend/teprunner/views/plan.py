    ...
    def create(self, request, *args, **kwargs):
        ...
        if task_status == "1":
            ...
            task_added = scheduler.add_job(func=run_plan_engine,
                                           ...
                                           id=str(plan.id),
                                           args=[
                                               project_id, plan.id, task_run_env, run_user_nickname, user_id],
                                           max_instances=1,
    ...
    def update(self, request, *args, **kwargs):
        ...
        if task_status == "1":
            ...
            task_updated = scheduler.add_job(func=run_plan_engine,
                                             ...
                                             id=str(plan_id),
                                             args=[
                                                 project_id, plan_id, task_run_env, run_user_nickname, user_id],
                                             max_instances=1,
                                             replace_existing=True)
        # 删除任务
        if task_status == "0":
    ...
    def list(self, request, *args, **kwargs):
        ...
        if keyword:
            try:
                ...
                case_id_query = Q()  # 非case_id
            case_desc_query = Q(case_id__in=[
                                case.id for case in Case.objects.filter(Q(desc__icontains=keyword))])
...
def case_result(request, *args, **kwargs):
    ...
    plan_result = PlanResult.objects.filter(
        plan_id=plan_id, case_id=case_id).order_by('-run_time')
