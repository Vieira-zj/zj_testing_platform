...
class ProjectPath:
    ...
    def __init__(self, project_id, env_name, user_id):
        ...
        # 通过 project_id + env_name + user_id 隔离运行环境
...
def delete_case_result(case_id, run_user_nickname):
    try:
        instance = CaseResult.objects.get(
            case_id=case_id, run_user_nickname=run_user_nickname)
        instance.delete()
...
def delete_plan_result(plan_id, case_id, run_user_nickname):
    ...
    """
    try:
        instance = PlanResult.objects.get(
            plan_id=plan_id, case_id=case_id, run_user_nickname=run_user_nickname)
        instance.delete()
    except ObjectDoesNotExist:
        pass
...
def run_case(request, *args, **kwargs):
    """
    run_case接口不会直接返回结果，前端通过WebSocket来查询结果的。
    """
    # 处理请求数据
    case_id = kwargs["pk"]
...
def run_plan(request, *args, **kwargs):
    ...
    plan_case_ids = [
        plan_case.case_id for plan_case in PlanCase.objects.filter(plan_id=plan_id)]
    ...
    for newest_case in pull_case_files(tests_dir, case_list):
        ...
        os.chdir(tests_dir)
        cmd = rf"pytest -s {filepath}"
        args = (pytest_subprocess, cmd, case_id,
                run_env, run_user_nickname, plan_id)
        thread_pool.submit(*args).add_done_callback(save_case_result)
