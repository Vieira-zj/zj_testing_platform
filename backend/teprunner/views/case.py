...
class CaseViewSet(ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        # 排除当前计划已经添加过的用例
        if exclude_plan_id:
            plan_case_ids = [plan_case.case_id for plan_case in PlanCase.objects.filter(
                plan_id=exclude_plan_id)]
    ...
    def update(self, request, *args, **kwargs):
        ...
        request.data["creatorNickname"] = Case.objects.get(
            id=case_id).creator_nickname
...
class CaseResultView(JsonWebsocketConsumer):
    ...
    def receive_json(self, content, **kwargs):
        ...
        while count <= timeout:
            ...
            # 没有查询到用例执行结果前，返回创建人和用例描述
            self.send_json(res)
            try:
                instances = CaseResult.objects.filter(
                    case_id=self.case_id).order_by('-run_time')
