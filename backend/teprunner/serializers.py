    ...
    def get_result(self, instance):
        ...
        try:
            case_result = CaseResult.objects.filter(
                case_id=case_id).order_by('-run_time')
            if case_result:
...
class CaseResultSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CaseResult
        fields = ["caseId", "result", "elapsed", "output",
...
class PlanSerializer(serializers.ModelSerializer):
    ...
    taskCrontab = serializers.CharField(
        source="task_crontab", allow_blank=True)
    ...
    class Meta:
        model = Plan
        fields = ["id", "name", "projectId", "taskStatus", "taskCrontab", "taskRunEnv", "caseNum",
                  "passedNum", "failedNum", "errorNum", "elapsed", "runEnv", "runUserNickname", "runTime"]
    ...
    def get_runTime(self, instance):
        ...
        run_time = ""
        if plan_results:
            run_time = plan_results.order_by(
                'run_time')[0].run_time.strftime("%Y-%m-%d %H:%M:%S")
        return run_time
