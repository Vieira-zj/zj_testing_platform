<template>
  <div>
    <wrap-component v-if="$route.name === 'case'"
                    tableTitle="全部用例">
      ...
      <el-form slot="form"
        ...
        <el-form-item label="API"
                      prop="api">
          <el-input v-model="searchForm.api"
                    placeholder="接口路径"
                    style="width: 400px"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button @click="searchCase"
                     type="primary"
      ...
      <el-table slot="table"
        ...
        <el-table-column prop="desc"
                         label="用例描述"
                         show-overflow-tooltip></el-table-column>
        <el-table-column prop="creatorNickname"
                         label="创建人"
                         width="80px"
                         align="center"
                         show-overflow-tooltip></el-table-column>
        <el-table-column prop="result"
          ...
          <template slot-scope="scope">
            <div :style="{ color: getResultColor(scope.row.result) }"
          ...
          <template slot-scope="scope">
            <div>
              <el-button type="success"
                         ...
                         size="mini"
                         plain
                         @click="gotoCaseResult('run', scope.row)"></el-button>
              <el-button type="info"
                         icon="el-icon-edit-outline"
                         size="mini"
                         plain
                         @click="gotoCaseEditor(scope.row)"></el-button>
              <el-button type="danger"
                         icon="el-icon-document-delete"
                         size="mini"
                         plain
                         @click="onDel('删除', scope.row)"></el-button>
              <el-button type="warning"
                         icon="el-icon-document-copy"
                         size="mini"
                         plain
                         @click="onCopy(scope.row.id)"></el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
    ...
    <writeDownDialog :writeDownDialogFormVisible.sync="writeDownDialogFormVisible"
