<template>
  <div>
    <project-env style="float: left; margin-right: 10px"
      ...
      <div style="clear: both"
        ...
        <div class="content-table">
          <el-table :header-cell-style="{
            ...
            <el-table-column label="计划名称"
                             prop="name"
                             show-overflow-tooltip></el-table-column>
            <el-table-column prop="caseNum"
                             label="用例"
                             width="50px"
                             align="center"
                             show-overflow-tooltip>
              <template slot-scope="scope">
                <div :style="numStyle('case', scope.row.caseNum)"
                     @click="gotoPlanResult(scope.row, 'case')">
                  {{ scope.row.caseNum }}
                </div>
              </template>
            </el-table-column>
            <el-table-column prop="failedPassed"
                             label="成功"
                             width="50px"
                             align="center"
                             show-overflow-tooltip>
              <template slot-scope="scope">
                <div :style="numStyle('passed', scope.row.passedNum)"
                     @click="gotoPlanResult(scope.row, 'passed')">
                  {{ scope.row.passedNum }}
                </div>
              </template>
            </el-table-column>
            <el-table-column prop="failedPassed"
                             ...
                             show-overflow-tooltip>
              <template slot-scope="scope">
                <div :style="numStyle('failed', scope.row.failedNum)"
                     @click="gotoPlanResult(scope.row, 'failed')">
                  {{ scope.row.failedNum }}
                </div>
              </template>
            </el-table-column>
            <el-table-column prop="failedPassed"
                             ...
                             show-overflow-tooltip>
              <template slot-scope="scope">
                <div :style="numStyle('error', scope.row.errorNum)"
                     @click="gotoPlanResult(scope.row, 'error')">
                  {{ scope.row.errorNum }}
                </div>
              </template>
            </el-table-column>
            <el-table-column prop="elapsed"
                             label="耗时"
                             width="60px "
                             align="center"
                             show-overflow-tooltip></el-table-column>
            <el-table-column prop="runEnv"
                             label="环境"
                             width="60px"
                             align="center"
                             show-overflow-tooltip></el-table-column>
            <el-table-column prop="runUserNickname"
                             label="运行人"
                             width="80px"
                             align="center"
                             show-overflow-tooltip></el-table-column>
            ...
            <el-table-column label="操作"
              ...
              <template slot-scope="scope">
                <div>
                  <el-button v-if="scope.row.caseNum > 0"
                             type="success"
        ...
        <div class="content-footer clear">
          <div class="block page-list self-right">
            <vue-pagination :currentPage="searchForm.page"
...
export default {
  ...
  methods: {
    ...
    runPlan(row) {
      ...
      let runUserNickname = JSON.parse(localStorage.getItem('userInfo'))
        .nickname
      let params = {
        curProjectId,
        runEnv,
      ...
      this.$http
        .post(`/teprunner/plans/${row.id}/run`, params)
        .then(({ data: { msg } }) => {
